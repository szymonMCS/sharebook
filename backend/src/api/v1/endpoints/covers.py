...
from src.api.deps import get_cover_service
from src.services.cover import CoverResult, get_cover_service as get_cover_service_impl
...
async def batch_covers(
    request: BatchRequest,
    background_tasks: BackgroundTasks = None,
    service=Depends(get_cover_service_impl),
):
    if len(request.books) > 50:
        raise HTTPException(400, "Max 50 books per request")
...
async def get_cover_file(isbn: str, service=Depends(get_cover_service_impl),):
    ...
    logger.info(f"[CoverEndpoint] Cover not cached, fetching: {isbn}")
    ...
    raise HTTPException(
        ...
        headers={"Location": f"https://covers.openlibrary.org/b/isbn/{isbn}-M.jpg"}
    )

def _convert_result(result: CoverResult) -> CoverResponse:
    status_map = {
