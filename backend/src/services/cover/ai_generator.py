...
try:
    ...
    logger.info("[CoverAIGenerator] OpenAI not available")
...
try:
    ...
    PIL_AVAILABLE = False
    logger.warning("[CoverAIGenerator] PIL not available - placeholder generation disabled")
...
class CoverAIGenerator:
    def __init__(self, openai_api_key: Optional[str] = None, use_dalle: bool = True, target_width: int = 180):
        ...
        if use_dalle and OPENAI_AVAILABLE and api_key:
            ...
            self.mode = "dalle"
            logger.info("[CoverAIGenerator] Using DALL-E mode")
        else:
            self.mode = "placeholder"
            if use_dalle and not OPENAI_AVAILABLE:
                logger.info("[CoverAIGenerator] OpenAI not installed, using placeholders")
            elif use_dalle and not api_key:
                logger.info("[CoverAIGenerator] No OpenAI API key, using placeholders")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as client:
                async with client.get(image_url, timeout=30) as img_response:
                    ...
                    return self._resize_image(data)
                    
        except Exception as e:
            logger.error(f"[CoverAIGenerator] DALL-E error: {e}")
            return await self._generate_placeholder(title, author)

    async def _generate_placeholder(self, title: str, author: str) -> Optional[bytes]:
        if not PIL_AVAILABLE:
            logger.error("[CoverAIGenerator] PIL not available, cannot generate placeholder")
            return None
